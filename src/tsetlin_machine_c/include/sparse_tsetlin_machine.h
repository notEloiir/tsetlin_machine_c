...
struct SparseTsetlinMachine {
	...
	uint8_t (*y_eq)(const struct SparseTsetlinMachine *stm, const void *y,
					const void *y_pred);
 ...
 * y_element_size: size of each element in the output vector (e.g., sizeof(uint32_t) for
 * labels being class indices) s: sensitivity, learning rate parameter > 1.0 (e.g., 10.0)
...
// Unless you know what you are doing, use the stm_set_* functions to set the desired
// components or leave them as default

// --- output_activation ---
// The raw output of a Tsetlin Machine are just summed up votes (stm->votes), of shape
...
void stm_oa_class_idx(const struct SparseTsetlinMachine *stm,
					  const void *y_pred); // y_size = 1

// Output is a binary vector of class predictions
// y_size = tm->num_classes, y_element_size = sizeof(uint8_t)
void stm_oa_bin_vector(const struct SparseTsetlinMachine *stm,
					   const void *y_pred); // y_size = tm->num_classes
...
void stm_feedback_bin_vector(struct SparseTsetlinMachine *stm, const uint8_t *X,
							 const void *y); // y_size = tm->num_classes
