...
struct StatelessTsetlinMachine {
	...
	uint8_t (*y_eq)(const struct StatelessTsetlinMachine *sltm, const void *y,
					const void *y_pred);
	...
	int8_t *feedback; // shape: flat (num_clauses, num_classes, 3) - clause-class feedback
					  // type strengths: 1a, 1b, 2
...
// Instead use sltm_load_dense to load (and prune) a pre-trained Tsetlin Machine from a
// bin file
...
// --- output_activation ---
// The raw output of a Tsetlin Machine are just summed up votes (sltm->votes), of shape
...
void sltm_oa_class_idx(const struct StatelessTsetlinMachine *sltm,
...
void sltm_oa_bin_vector(const struct StatelessTsetlinMachine *sltm,
