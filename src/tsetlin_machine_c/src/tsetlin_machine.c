...
struct TsetlinMachine *tm_create(uint32_t num_classes, uint32_t threshold,
	...
	struct TsetlinMachine *tm =
		(struct TsetlinMachine *)malloc(sizeof(struct TsetlinMachine));
	if (tm == NULL) {
	...
	tm->ta_state =
		(int8_t *)malloc(num_clauses * num_literals * 2 *
						 sizeof(int8_t)); // shape: flat (num_clauses, num_literals, 2)
	if (tm->ta_state == NULL) {
		perror("Memory allocation failed");
		tm_free(tm);
		return NULL;
...
struct TsetlinMachine *tm_load(const char *filename, uint32_t y_size,
	...
	// Read weights
	size_t weights_read =
		fread(tm->weights, sizeof(int16_t), num_clauses * num_classes, file);
	...
	size_t states_read =
		fread(tm->ta_state, sizeof(int8_t), num_clauses * num_literals * 2, file);
...
struct TsetlinMachine *tm_load_fbs(const char *filename, uint32_t y_size,
	...
	// Extract weights
	TsetlinMachine_ClauseWeightsTensor_table_t weights =
		TsetlinMachine_Model_clause_weights(model);
	if (!weights) {
	...
	// Extract states
	TsetlinMachine_AutomatonStatesTensor_table_t states =
		TsetlinMachine_Model_automaton_states(model);
	if (!states) {
	...
	struct TsetlinMachine *tm =
		tm_create(num_classes, threshold, num_literals, num_clauses, max_state, min_state,
				  boost_true_positive_feedback, y_size, y_element_size, s, 42);
	if (!tm) {
		fprintf(stderr, "tm_create failed\n");
		free(buffer);
		return NULL;
	...
	flatbuffers_int16_vec_t weights_vec =
		TsetlinMachine_ClauseWeightsTensor_weights(weights);
	...
	flatbuffers_int8_vec_t states_vec =
		TsetlinMachine_AutomatonStatesTensor_states(states);
...
void tm_save_fbs(struct TsetlinMachine *tm, const char *filename) {
	...
	// Create weights
	flatbuffers_int16_vec_ref_t weights_vec = flatbuffers_int16_vec_create(
		&builder, tm->weights, (size_t)tm->num_clauses * tm->num_classes);
	uint32_t weights_shape_data[] = {tm->num_clauses, tm->num_classes};
	flatbuffers_uint32_vec_ref_t weights_shape_vec =
		flatbuffers_uint32_vec_create(&builder, weights_shape_data, 2);
	...
	TsetlinMachine_ClauseWeightsTensor_ref_t clause_weights =
		TsetlinMachine_ClauseWeightsTensor_end(&builder);

	// Create states
	flatbuffers_int8_vec_ref_t states_vec = flatbuffers_int8_vec_create(
		&builder, tm->ta_state, (size_t)tm->num_clauses * tm->num_literals * 2);
	uint32_t states_shape_data[] = {tm->num_clauses, tm->num_literals, 2};
	flatbuffers_uint32_vec_ref_t states_shape_vec =
		flatbuffers_uint32_vec_create(&builder, states_shape_data, 3);
	...
	TsetlinMachine_AutomatonStatesTensor_ref_t automaton_states =
		TsetlinMachine_AutomatonStatesTensor_end(&builder);
...
void tm_initialize(struct TsetlinMachine *tm) {
	...
	// pairs of positive and negative literals randomly (-1, 0) or (0, -1) if mid_state is
	// 0
	for (uint32_t clause_id = 0; clause_id < tm->num_clauses; clause_id++) {
		for (uint32_t literal_id = 0; literal_id < tm->num_literals; literal_id++) {
			if (prng_next_float(&(tm->rng)) <= 0.5) {
				// positive literal
				tm->ta_state[(((clause_id * tm->num_literals) + literal_id) * 2) + 0] =
					tm->mid_state - 1;
				// negative literal
	...
	for (uint32_t clause_id = 0; clause_id < tm->num_clauses; clause_id++) {
		for (uint32_t class_id = 0; class_id < tm->num_classes; class_id++) {
			tm->weights[(clause_id * tm->num_classes) + class_id] =
				1 - 2 * (prng_next_float(&(tm->rng)) <= 0.5);
		}
	}
}
 ...
 */
static inline void calculate_clause_output(struct TsetlinMachine *tm, const uint8_t *X,
										   uint8_t skip_empty) {
	...
	for (uint32_t clause_id = 0; clause_id < tm->num_clauses; clause_id++) {
		...
		for (uint32_t literal_id = 0; literal_id < tm->num_literals; literal_id++) {
			uint8_t action_include = action(
				tm->ta_state[(((clause_id * tm->num_literals) + literal_id) * 2) + 0],
				tm->mid_state);
			uint8_t action_include_negated = action(
				tm->ta_state[(((clause_id * tm->num_literals) + literal_id) * 2) + 1],
				tm->mid_state);

			empty_clause = (empty_clause && !(action_include || action_include_negated));

			if ((action_include == 1 && X[literal_id] == 0) ||
				(action_include_negated == 1 && X[literal_id] == 1)) {
 ...
 */
static inline void type_1a_feedback(struct TsetlinMachine *tm, const uint8_t *X,
	...
	if (tm->weights[clause_id * tm->num_classes + class_id] >= 0) {
		tm->weights[clause_id * tm->num_classes + class_id] +=
			min(feedback_strength,
				SHRT_MAX - tm->weights[clause_id * tm->num_classes + class_id]);
	} else {
		tm->weights[clause_id * tm->num_classes + class_id] -=
			min(feedback_strength,
				-(SHRT_MIN - tm->weights[clause_id * tm->num_classes + class_id]));
	}
		...
		if (X[literal_id] == 1) {
			...
			tm->ta_state[(((clause_id * tm->num_literals) + literal_id) * 2)] +=
				min(tm->max_state -
						tm->ta_state[(((clause_id * tm->num_literals) + literal_id) * 2)],
					feedback_strength) *
				((tm->boost_true_positive_feedback == 1 ||
 ...
 */
static inline void type_2_feedback(struct TsetlinMachine *tm, const uint8_t *X,
								   uint32_t clause_id, uint32_t class_id) {
	uint8_t feedback_strength = 1;

	tm->weights[clause_id * tm->num_classes + class_id] +=
		tm->weights[clause_id * tm->num_classes + class_id] >= 0 ? -feedback_strength
																 : feedback_strength;
	...
	for (uint32_t literal_id = 0; literal_id < tm->num_literals; literal_id++) {
		tm->ta_state[(((clause_id * tm->num_literals) + literal_id) * 2)] +=
			min(tm->max_state -
				...
				feedback_strength) *
			(0 ==
				 action(tm->ta_state[(((clause_id * tm->num_literals) + literal_id) * 2)],
						tm->mid_state) &&
			 0 == X[literal_id]);
...
void tm_train(struct TsetlinMachine *tm, const uint8_t *X, const void *y, uint32_t rows,
	...
	for (uint32_t epoch = 0; epoch < epochs; epoch++) {
		for (uint32_t row = 0; row < rows; row++) {
			...
			void *y_row =
				(void *)((uint8_t *)y + (row * tm->y_size * tm->y_element_size));
 ...
 */
void tm_predict(struct TsetlinMachine *tm, const uint8_t *X, void *y_pred,
				uint32_t rows) {
	for (uint32_t row = 0; row < rows; row++) {
		const uint8_t *X_row = X + (row * tm->num_literals);
		void *y_pred_row =
			(void *)(((uint8_t *)y_pred) + (row * tm->y_size * tm->y_element_size));
...
void tm_feedback_class_idx(struct TsetlinMachine *tm, const uint8_t *X, const void *y) {
	...
	int32_t votes_clipped_positive =
		clip(tm->votes[positive_class], (int32_t)tm->threshold);
	float update_probability_positive =
		((float)tm->threshold - (float)votes_clipped_positive) /
		(float)(2 * tm->threshold);
	...
	for (uint32_t class_id = 0; class_id < tm->num_classes; class_id++) {
		if (class_id != positive_class) {
			sum_votes_clipped_negative +=
				clip(tm->votes[class_id], (int32_t)tm->threshold) +
				(int32_t)tm->threshold;
		}
	}
	if (sum_votes_clipped_negative == 0)
		return;
	int32_t random_vote_negative =
		prng_next_uint32(&(tm->rng)) % sum_votes_clipped_negative;
	int32_t accumulated_votes = 0;
	for (uint32_t class_id = 0; class_id < tm->num_classes; class_id++) {
		if (class_id != positive_class) {
			accumulated_votes += clip(tm->votes[class_id], (int32_t)tm->threshold) +
								 (int32_t)tm->threshold;
	...
	int32_t votes_clipped_negative =
		clip(tm->votes[negative_class], (int32_t)tm->threshold);
	float update_probability_negative =
		((float)votes_clipped_negative + (float)tm->threshold) /
		(float)(2 * tm->threshold);
...
void tm_feedback_bin_vector(struct TsetlinMachine *tm, const uint8_t *X, const void *y) {
	...
	int32_t random_vote_positive =
		prng_next_uint32(&(tm->rng)) % sum_votes_clipped_positive;
